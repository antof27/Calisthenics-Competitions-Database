...
--
DELIMITER $$
CREATE TRIGGER `cancellaCompetizioni` BEFORE INSERT ON `Competizione` FOR EACH ROW BEGIN
  DECLARE numCompetizioni INT;
  SELECT COUNT(*) INTO numCompetizioni FROM Competizione
  WHERE IDStruttura = NEW.IDStruttura AND Data = NEW.Data;

  IF numCompetizioni > 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'There is already a competition scheduled for this date and location.';
  END IF;
END
$$
DELIMITER ;
...
CREATE TRIGGER `verificaAllenatore` BEFORE INSERT ON `Giudice` FOR EACH ROW BEGIN
   DECLARE numAllenatori INT;
   
   SELECT COUNT(*) INTO numAllenatori
   FROM Allenatore
   WHERE IDAllenatore = NEW.IDGiudice;
   
   IF numAllenatori > 0 THEN
      SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'This judge is also a coach. Coaches cannot serve as judges.';
   END IF;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `eliminaAtletaPosizionamento` BEFORE INSERT ON `Posizionamento` FOR EACH ROW BEGIN
   DECLARE numAtleti INT;
   
   SELECT COUNT(*) INTO numAtleti
   FROM Posizionamento
   WHERE IDGara = NEW.IDGara AND Posizione = NEW.Posizione;
   
   IF numAtleti > 0 THEN
      SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'There is already an athlete with this position in the same race.';
   END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `erratoPosizionamento` BEFORE INSERT ON `Posizionamento` FOR EACH ROW BEGIN
...
CREATE TRIGGER `limitRegolamenti` AFTER INSERT ON `Regolamento` FOR EACH ROW BEGIN
   ...
   IF numRegolamenti >= 5 THEN
      INSERT INTO WarningLog (msg, Timestamp) VALUES ('There are already 5 regulations for this category and type. Consider deleting one before adding a new one.', CURRENT_TIMESTAMP);
   END IF;
END
$$
DELIMITER ;
...
--
ALTER TABLE `Competizione`
  MODIFY `IDCompetizione` int(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;
...
--
ALTER TABLE `WarningLog`
  MODIFY `IDWarning` int(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

...
--
-- Host: localhost
-- Generation Time: Dec 31, 2022 at 12:43 PM
...
CREATE TABLE `Allenatore` (
  ...
  `nomePalestra` varchar(30) NOT NULL
...
INSERT INTO `Allenatore` (`IDAllenatore`, `IDAtleta`, `nomePalestra`) VALUES
('JRMFZC69T06L537B', 'RVRDFB70R08I381R', 'Athlon'),
('PRVBCL46S52G382G', 'FMPCVD63A29L384Q', 'Virgin'),
('PRVBCL46S52G382G', 'WQKJTF77A69B869L', 'Fitness And Fight');
...
INSERT INTO `Atleta` (`IDAtleta`, `Genere`, `Età`, `Peso`, `Altezza`) VALUES
('FMPCVD63A29L384Q', 'F', 33, 60, 171),
('LBHDRX82D44E892E', 'M', 32, 82, 159),
('NRQLPB51D02Z600V', 'M', 24, 67, 174),
('NVOYDS94R15G536D', 'M', 27, 102, 176),
('RVRDFB70R08I381R', 'F', 18, 53, 165),
('WQKJTF77A69B869L', 'M', 24, 62, 175);
...
INSERT INTO `Giudice` (`IDGiudice`, `IDGara`) VALUES
('DHYPGB66H08E707I', 5),
('SPSPNQ37M22G616K', 11),
('SPSPNQ37M22G616K', 12);
...
CREATE TRIGGER `verificaAllenatore` BEFORE INSERT ON `Giudice` FOR EACH ROW BEGIN
   ...
   IF Allenatore > 0 THEN
      DELETE FROM Giudice
      WHERE IDGiudice = NEW.IDGiudice AND IDGara = NEW.IDGara;
   END IF;
END
$$
DELIMITER ;
...
INSERT INTO `OrganizzatoreEvento` (`IDOrganizzatore`, `IDCompetizione`) VALUES
('NDSHHS58A54D218L', 5),
('PNFSCH55P28B739N', 2),
('PNFSCH55P28B739N', 5),
('PNFSCH55P28B739N', 6);
...
INSERT INTO `Regolamento` (`IDRegolamento`, `IDTipologia`, `IDCategoria`, `annoRegolamento`) VALUES
(1, 2, 1, 2019),
(2, 4, 6, 2020),
(3, 1, 6, 2020),
(4, 5, 9, 2021),
...
CREATE TRIGGER `limitRegolamenti` AFTER INSERT ON `Regolamento` FOR EACH ROW BEGIN
   DECLARE num_regolamenti INT;
   
   SELECT COUNT(*) INTO num_regolamenti
   FROM Regolamento
   WHERE IDCategoria = NEW.IDCategoria AND IDTipologia = NEW.IDTipologia;
   
   IF num_regolamenti > 5 THEN
...
INSERT INTO `Spettatore` (`IDSpettatore`, `IDCompetizione`) VALUES
('CDHVSM36H68D279F', 1),
('GFQPDR63A59G459V', 1),
('GFQPDR63A59G459V', 3),
('GHBVBV46P22L217T', 5),
...
--
DELIMITER $$
CREATE TRIGGER `inserimentoSpettatore` AFTER INSERT ON `Spettatore` FOR EACH ROW BEGIN
   UPDATE Competizione
...
ALTER TABLE `Allenatore`
  ADD PRIMARY KEY (`IDAllenatore`,`IDAtleta`),
  ADD KEY `test2` (`IDAtleta`);
...
ALTER TABLE `Regolamento`
  ADD PRIMARY KEY (`IDRegolamento`),
  ADD KEY `t11` (`IDCategoria`),
  ADD KEY `t22` (`IDTipologia`);
...
--
ALTER TABLE `Gara`
  MODIFY `IDGara` int(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=15;

--
-- AUTO_INCREMENT for table `Regolamento`
--
ALTER TABLE `Regolamento`
  MODIFY `IDRegolamento` int(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;
...
ALTER TABLE `Allenatore`
  ...
  ADD CONSTRAINT `test2` FOREIGN KEY (`IDAtleta`) REFERENCES `Atleta` (`IDAtleta`);
...
ALTER TABLE `Giudice`
  ...
  ADD CONSTRAINT `tt2` FOREIGN KEY (`IDGara`) REFERENCES `Gara` (`IDGara`);

--
-- Constraints for table `OrganizzatoreEvento`
--
ALTER TABLE `OrganizzatoreEvento`
  ADD CONSTRAINT `test01` FOREIGN KEY (`IDCompetizione`) REFERENCES `Competizione` (`IDCompetizione`),
  ADD CONSTRAINT `test02` FOREIGN KEY (`IDOrganizzatore`) REFERENCES `Persona` (`CF`);
...
--
ALTER TABLE `Spettatore`
  ADD CONSTRAINT `testt4` FOREIGN KEY (`IDCompetizione`) REFERENCES `Competizione` (`IDCompetizione`),
  ADD CONSTRAINT `testt5` FOREIGN KEY (`IDSpettatore`) REFERENCES `Persona` (`CF`);
COMMIT;
